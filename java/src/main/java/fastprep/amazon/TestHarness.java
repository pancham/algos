...
public class TestHarness {
    ...
    // Helper method to check if we can distribute servers into numGroups groups
    private static boolean canDistribute(Map<Integer, Integer> frequencyMap, int n, int numGroups) {
    ...
    public static int findMinimumGroups(List<Integer> security) {
        ...
        while (left <= right) {
            ...
            if (canDistribute(frequencyMap, n, mid)) {
                result = mid; // Found a valid solution, try smaller
                right = mid - 1;
            } else {
                left = mid + 1; // Need more groups
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                "Passed: " + passedTests + " (" + String.format("%.2f", 100.0 * passedTests / totalTests) + "%)");
        System.out.println(
                "Failed: " + failedTests + " (" + String.format("%.2f", 100.0 * failedTests / totalTests) + "%)");
        System.out.println("=".repeat(50));
