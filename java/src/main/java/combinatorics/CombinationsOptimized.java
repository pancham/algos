 ...
 *
 *   1. Bitmask iteration for subsets — avoids recursion stack; O(1) extra space during traversal
 *      vs O(n) call stack in DFS.
 *   2. Pre-allocated result lists — sizes ArrayList to the exact output count upfront,
 *      eliminating repeated internal array resizing.
 *   3. Lazy iterator variants — yield one result at a time, reducing peak memory from
 *      O(n * 2^n) / O(k * C(n,k)) down to O(n) / O(k).
 *
 * Asymptotic time complexity is unchanged — the output-sensitive lower bound cannot be beaten
 * because you must write every result to memory regardless of the algorithm used.
...
public class CombinationsOptimized {
     ...
     *
     * Duplicate rule: for a sorted array, a duplicate element at position i is only valid
     * if the element at position i-1 is also selected. This mirrors the DFS skip rule
    ...
    public List<List<Integer>> subsets(int[] nums) {
        ...
        for (int mask = 0; mask < total; mask++) {
            if (!isValidMask(mask, sorted)) continue;
            ...
            for (int i = 0; i < n; i++) {
                if ((mask >> i & 1) == 1) subset.add(sorted[i]);
            }
            result.add(subset);
        }
    ...
    /**
     * A mask is invalid if it selects a duplicate element without selecting its predecessor.
     * e.g. sorted=[1,2,2]: mask 100 selects index 2 (second 2) but not index 1 (first 2),
    ...
    /**
     * Lazy variant: yields one subset at a time without materializing the full output.
     *
    ...
    public Iterable<List<Integer>> subsetsLazy(int[] nums) {
        ...
        return () -> new Iterator<List<Integer>>() {
            ...
            private int findNextValid(int from) {
                while (from < total && !isValidMask(from, sorted)) from++;
                return from;
            }
        };
    }
    ...
    /**
     * Generates all unique combinations of size k using DFS with pruning and pre-allocation.
     *
     * Same DFS + backtracking algorithm as Combinations.java, with two improvements:
     *   - result list pre-allocated to C(n,k) via the binomial() helper.
     *   - each combination list pre-allocated to k, avoiding resizing during backtracking.
     *
     * Pruning (carried over from baseline): the guard
     *   current.size() + (nums.length - start) < k
     * cuts branches where not enough elements remain to reach size k. Does not change the
     ...
     *      Unlike subsets, no identity is needed — size is fixed, so it is direct multiplication.
     *
     *   Total: C(n,k) + k * C(n,k)
     *        = C(n,k) * (1 + k)
    ...
    public List<List<Integer>> combine(int[] nums, int k) {
        if (nums == null || k < 0 || k > nums.length) return new ArrayList<>();
    ...
    /**
     * Lazy variant: yields one combination at a time without materializing the full output.
     *
     * Maintains an index array [c0, c1, ..., c_{k-1}] where each ci is a position in sorted[].
     ...
     * The advance() loop skips invalid arrays automatically, mirroring the DFS skip rule.
     *
     * Complexity:
     *   Time  — O(k * C(n,k)) total across all next() calls.
     *   Space — O(k) peak: only the index array and one combination in memory at any moment,
     *            vs O(k * C(n,k)) for the eager version.
     */
    public Iterable<List<Integer>> combineLazy(int[] nums, int k) {
        if (nums == null || k < 0 || k > nums.length) return Collections.emptyList();
        ...
        return () -> new Iterator<List<Integer>>() {
            ...
            public List<Integer> next() {
                ...
                for (int idx : indices) combo.add(sorted[idx]);
                advance();
                return combo;
            }
            ...
            private void advance() {
                while (true) {
                    ...
                    if (isValidIndices(indices)) return;
                }
            }
            ...
            private boolean isValidIndices(int[] idx) {
                for (int j = 0; j < idx.length; j++) {
                    ...
                    if (pos > 0 && sorted[pos] == sorted[pos - 1]) {
                        if (j == 0 || idx[j - 1] != pos - 1) return false;
                    }
                }
                return true;
            }
    ...
    /**
     * Computes C(n, k) iteratively using the recurrence C(n,k) = C(n,k-1) * (n-k+1) / k.
     * Avoids large intermediate factorials by interleaving multiply and divide.
     */
    private int binomial(int n, int k) {
        if (k > n - k) k = n - k; // C(n,k) == C(n, n-k); use the smaller k
        long result = 1;
        for (int i = 0; i < k; i++) {
